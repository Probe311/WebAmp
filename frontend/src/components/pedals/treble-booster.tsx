...
export const TrebleBoosterControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const TrebleBoosterPedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default TrebleBoosterPedal
