...
export const LightBoostControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const LightBoostPedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default LightBoostPedal
