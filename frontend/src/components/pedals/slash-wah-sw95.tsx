...
export const SlashWahSw95Controls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const SlashWahSw95Pedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default SlashWahSw95Pedal
