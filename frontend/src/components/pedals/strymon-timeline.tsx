...
const pedalId = 'strymon-timeline'
...
export const StrymonTimelineControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const StrymonTimelinePedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default StrymonTimelinePedal
