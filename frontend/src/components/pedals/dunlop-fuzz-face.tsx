...
const pedalId = 'dunlop-fuzz-face'
...
export const DunlopFuzzFaceControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const DunlopFuzzFacePedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default DunlopFuzzFacePedal
