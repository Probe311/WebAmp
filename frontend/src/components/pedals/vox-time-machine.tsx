...
const pedalId = 'vox-time-machine'
...
export const VoxTimeMachineControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const VoxTimeMachinePedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default VoxTimeMachinePedal
