...
export const MxrPhase90Controls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const MxrPhase90Pedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default MxrPhase90Pedal
