...
export const IbanezJeminiControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const IbanezJeminiPedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default IbanezJeminiPedal
