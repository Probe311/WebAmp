...
const pedalId = 'morley-bad-horsie'
...
export const MorleyBadHorsieControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const MorleyBadHorsiePedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default MorleyBadHorsiePedal
