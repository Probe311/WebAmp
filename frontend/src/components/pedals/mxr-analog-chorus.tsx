...
const pedalId = 'mxr-analog-chorus'
...
export const MxrAnalogChorusControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const MxrAnalogChorusPedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default MxrAnalogChorusPedal
