...
const pedalId = 'memory-man-delay'
...
export const MemoryManDelayControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const MemoryManDelayPedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default MemoryManDelayPedal
