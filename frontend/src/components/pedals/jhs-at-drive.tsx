...
export const JhsAtDriveControls = ({
  values = {},
  onChange,
}: PedalComponentProps) => {
...
export const JhsAtDrivePedal = ({ values = {}, onChange, bypassed = false }: PedalComponentProps) => {
  const model = pedalLibrary.find((p) => p.id === pedalId)
  if (!model) return null
  const params = model.parameters
...
export default JhsAtDrivePedal
